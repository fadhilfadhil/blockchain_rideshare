...
if(isset($_POST["submit"])) {
  $check = getimagesize($_FILES["pp_driver"]["tmp_name"]);
  if($check !== false  ) {
...
// Check file size
if ($_FILES["pp_driver"]["size"] > 1000 * 1000){
...
// if everything is ok, try to upload file
    ...
    $image = $_FILES['pp_driver']['tmp_name'];
    $fo = fopen($_FILES['pp_driver']['tmp_name'], "r");
    $imageContent = fread($fo, filesize($image));
    $hashpp = $ipfs->add($imageContent);
    ...
    $contract->at($contractAddress)->send('update_pict',$username,$hashpp,[
        'from' => $fromAccount,
        'gas' => '0x200b20'
    ], function ($err, $result)  {
        if ($err !== null) {
