...
$long = "'" . $long;
$lat = "'" . $lat;
$contractAddressD = $addressdriver;//'0xfb1ca0a9b68b6973d4af4033e4fc2403850ac600';
$contractD->at($contractAddressD)->call('get_driver',$username,[
    'from' => $fromAccount,
    'gas' => '0x200b20'
], function ($err, $result) use ($contract,$contractAddress,$fromAccount,$long,$lat,$username  ) {
    if ($err !== null) {
        echo 'error';
    }
    if (isset($result)) {
        $hp = $result[3]; //3 hp
        $contract->at($contractAddress)->send('driver_login',$username,$long,$lat,$hp,[
            'from' => $fromAccount,
            'gas' => '0x200b20'
        ], function ($err, $result)  {
            if ($err !== null) {
