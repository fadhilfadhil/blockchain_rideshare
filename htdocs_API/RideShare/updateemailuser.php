...
$contract->at($contractAddress)->call('penumpang_login',$username,$password,[
    'from' => $fromAccount,
    'gas' => '0x200b20'
], function ($err, $result) use ($username,$password,$email,$contract,$contractAddress,$fromAccount) {
    if ($err !== null) {
        echo 'error';
    }
    if (isset($result)) {
        if ($username == $result[1]){
            ...
            $contract->at($contractAddress)->send('change_email_penumpang',$username,$password,$email,[
                'from' => $fromAccount,
                'gas' => '0x300b20'
            ], function ($err, $result)  {
                if ($err !== null) {
